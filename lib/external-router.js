  ...
  function reconnect () {
    ...
    Object.keys(routes).forEach(function (from) {
      ...
      if (output && Array.isArray(target)) {
        ...
        var outputWithGain = context.audio.createGain()
        outputWithGain.gain.value = volume && volume() || 1
        output.connect(outputWithGain)

        target.forEach(function (to) {
          if (to && typeof to === 'string') {
            if (to === '$default') {
              outputWithGain.connect(context.output)
              routed = true
            } else {
              to = to.split('#')
              ...
              if (destinationSlot && destinationSlot.input) {
                outputWithGain.connect(destinationSlot.input)
                routed = true
              }
            }
          }
